...
test_guard_warnings() {
  ...
  grep -E 'repair missing watcher supervision|repair a missing or failed watcher cycle' "$err" >/dev/null || fail "guard banner missing the harness-aware fix command"
...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  mark_pr_check_migration_complete "$state"
  node -e 'process.on("SIGTERM", () => {}); process.stdout.write("ready\n"); setTimeout(() => {}, 300000)' > "$ready" &
  peer=$!
  i=0
  while [ "$i" -lt 50 ]; do
    grep -qxF ready "$ready" 2>/dev/null && break
    sleep 0.1
    i=$((i + 1))
  done
  grep -qxF ready "$ready" 2>/dev/null || fail "TERM-resistant peer did not finish installing its signal handler"
