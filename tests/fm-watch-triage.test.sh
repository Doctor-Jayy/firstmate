...
test_crew_absorb_class_classifier() {
  ...
  FM_FAKE_CREW_STATE='state: done · source: run-step · checks green'
  [ "$(crew_absorb_class a)" = terminal ] || fail "terminal run-step did not supersede an older pause"
  FM_FAKE_CREW_STATE='state: working · source: status-log · working: compiling'
  ...
  unset FM_FAKE_CREW_STATE
  pass "crew_absorb_class: working/paused/terminal/none from one read; crew_is_paused and crew_is_provably_working agree"
}
...
test_nonterminal_stale_paused_done_run_surfaces_once_per_hash() {
  ...
  reap "$pid"
  unset FM_FAKE_CREW_STATE FM_FAKE_CREW_STATE_LOG FM_FAKE_TMUX_WINDOW FM_FAKE_TMUX_CAPTURE FM_FAKE_TMUX_CURRENT_COMMAND FM_STALE_ESCALATE_SECS
