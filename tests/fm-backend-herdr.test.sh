...
test_presentation_lock_wait_override_is_validated() {
  local lock_source out
  lock_source=$(sed -n '/^spawn_herdr_presentation_order_lock_acquire()/,/^spawn_herdr_presentation_order_lock_release()/p' "$ROOT/bin/fm-spawn.sh" | sed '$d')
  out=$(LOCK_SOURCE="$lock_source" bash -c '
    eval "$LOCK_SOURCE"
    fm_backend_herdr_presentation_session_lock_path() { printf "%s" /tmp/fm-test-lock; }
    sleep() { :; }
    fm_lock_try_acquire() {
      attempts=$((attempts + 1))
      [ "$attempts" -eq 55 ]
    ...
    attempts=0
    FM_BACKEND_HERDR_PRESENTATION_LOCK_WAIT_SECONDS=6
    spawn_herdr_presentation_order_lock_acquire fmtest || exit 1
    printf "override=%s " "$attempts"
    attempts=0
    FM_BACKEND_HERDR_PRESENTATION_LOCK_WAIT_SECONDS=invalid
    if spawn_herdr_presentation_order_lock_acquire fmtest; then
      exit 1
    fi
    printf "invalid=%s" "$attempts"
  ')
  [ "$out" = "override=55 invalid=50" ] \
    || fail "presentation lock wait override was not positive-integer validated: $out"
  pass "herdr presentation lock: validated wait override leaves the five-second default intact"
}
...
classify_pi_composer_fixture() {  # <fixture> [agent-identity] [tail-max]
  ...
  FM_COMPOSER_FIXTURE="$fixture" FM_PI_AGENT_IDENTITY="$identity" \
    ...
    /bin/bash -c '
      ...
      fm_backend_herdr_agent_identity_raw() { printf "%s\tidle" "$FM_PI_AGENT_IDENTITY"; }
...
test_composer_state_ghost_placeholder_is_empty() {
  ...
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" LC_ALL=C LANG=C \
    /bin/bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_composer_state default:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "the known ghost placeholder 'Type a message...' should read as empty, got '$out'"
  pass "fm_backend_herdr_composer_state: the UTF-8-prefixed ghost placeholder reads empty under a C locale"
}
...
test_events_capable_accepts_large_schema_under_pipefail_quietly() {
  local dir log resp fb earlybin schema out status
  dir="$TMP_ROOT/events-capable-large-schema"; mkdir -p "$dir/responses" "$dir/earlybin"
  log="$dir/log"; resp="$dir/responses"; earlybin="$dir/earlybin"; schema="$resp/2.out"; : > "$log"
  printf '{"client":{"version":"0.7.5","protocol":16}}\n' > "$resp/1.out"
  {
    printf '{"methods":{"events.subscribe":{},"pane.agent_status_changed":{}},"padding":"'
    dd if=/dev/zero bs=1024 count=220 2>/dev/null | tr '\0' x
    printf '"}\n'
  } > "$schema"
  cat > "$earlybin/grep" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$earlybin/grep"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$earlybin:$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" FM_HERDR_SCRIPT_STATUS=1 \
    /bin/bash -o pipefail -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_events_capable default' "$ROOT" 2>&1)
  status=$?
  expect_code 0 "$status" "events capability must not fail when an early successful schema reader closes a pipe"
  [ -z "$out" ] || fail "events capability probe must stay quiet, got: $out"
  pass "fm_backend_herdr_events_capable: large schemas stay capable and quiet under caller pipefail"
}

test_events_capable_fails_closed_when_schema_surface_is_missing() {
  local dir log resp fb out status
  dir="$TMP_ROOT/events-capable-missing-surface"; mkdir -p "$dir/responses"
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '{"client":{"version":"0.7.5","protocol":16}}\n' > "$resp/1.out"
  printf '{"methods":{"events.subscribe":{}}}\n' > "$resp/2.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" FM_HERDR_SCRIPT_STATUS=1 \
    /bin/bash -o pipefail -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_events_capable default' "$ROOT" 2>&1)
  status=$?
  [ "$status" -ne 0 ] || fail "events capability must fail closed when pane.agent_status_changed is absent"
  [ -z "$out" ] || fail "failed-closed events capability probe must stay quiet, got: $out"
  pass "fm_backend_herdr_events_capable: missing schema surface fails closed and quiet"
}
