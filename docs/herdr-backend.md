...
It also pins byte-exact stripping of the multibyte `❯` and `›` prompt prefixes under macOS Bash 3.2 in a C locale, with adapter-level C-locale coverage in the existing tmux, herdr, Orca, and cmux test suites.
...
After the backend selects the middle row, the existing `fm_composer_strip_ghost` and `fm_composer_classify_content` functions remain the only owners of empty-versus-pending content classification, with Pi selecting the classifier's literal-glyph mode because its separator-framed row has no harness prompt glyph.
Submit confirmation remains on native Herdr agent state.

**Deterministic regressions.** `tests/fm-backend-herdr.test.sh` pins the exact 53-glyph empty frame as `empty`, synthetic text and each of `>`, `$`, `%`, `#`, `❯`, and `›` as `pending`, and every ambiguous shape above as `unknown`.
`tests/fm-daemon.test.sh` routes `inject_msg` through the real Herdr classifier and proves one submit for the empty Pi frame and no submit for text or glyph-only Pi drafts.
