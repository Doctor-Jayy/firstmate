...
It also pins byte-exact stripping of the multibyte `❯` and `›` prompt prefixes under macOS Bash 3.2 in a C locale, where character-count patterns like `${content#??}` would otherwise split the glyph and misclassify an empty composer as `pending`.
`FM_BACKEND_HERDR_BARE_PROMPT_RE` is an alternation (`^(❯|›)`), not a bracket class, for the same reason: a bracket class byte-decomposes those glyphs under `LC_ALL=C`, which `tests/fm-backend-herdr.test.sh`'s `test_composer_state_bare_prompt_is_empty_under_c_locale` pins directly.
