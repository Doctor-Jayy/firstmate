...
fm_backend_herdr_pi_separator_row() {  # <plain-row>
  ...
  row="${row#"${row%%[![:space:]]*}"}"
  row="${row%"${row##*[![:space:]]}"}"
  # Keep the eight-glyph minimum byte-safe under macOS Bash 3.2 in a C locale.
  case "$row" in
    '────────'*) ;;
    *) return 1 ;;
  esac
...
fm_backend_herdr_pi_composer_find() {  # <ansi-capture>
  local cap=$1 line plain open=0 lines=0 candidate="" max row=0 open_separator=""
  ...
  FM_BACKEND_HERDR_PI_CAPTURE_LINES=0
  ...
  while IFS= read -r line; do
    ...
    if fm_backend_herdr_pi_separator_row "$plain"; then
      ...
      if [ "$open" -eq 1 ]; then
        ...
        if [ "$lines" -le "$max" ] && [ "$plain" = "$open_separator" ]; then
...
$cap
EOF
  FM_BACKEND_HERDR_PI_CAPTURE_LINES=$row
}
...
fm_backend_herdr_composer_state() {  # <target> -> empty|pending|unknown
  ...
  local identity agent agent_status row=0 generic_line=0 pi_tail_max
  pi_tail_max=$FM_BACKEND_HERDR_PI_COMPOSER_TAIL_MAX
  ...
  if [ "$FM_BACKEND_HERDR_PI_PAIR_FOUND" -eq 1 ] \
     && [ "$FM_BACKEND_HERDR_PI_PAIR_LINE" -gt "$generic_line" ]; then
    if [ "$FM_BACKEND_HERDR_PI_PAIR_LINE" -ne "$FM_BACKEND_HERDR_PI_LAST_SEPARATOR_LINE" ] \
       || [ $((FM_BACKEND_HERDR_PI_CAPTURE_LINES - FM_BACKEND_HERDR_PI_PAIR_LINE)) -gt "$pi_tail_max" ]; then
      found=0
    else
  ...
  if [ "$shape" = bordered ]; then
    ...
    # composer container. Every nonempty Pi row is a draft, including text that
    # resembles another harness's prompt glyph or placeholder.
...
fm_backend_herdr_events_capable() {  # <session>
  ...
  # Avoid an early-exiting reader on this large value: a pipefail caller can
  # otherwise turn the writer's expected SIGPIPE into a false negative.
