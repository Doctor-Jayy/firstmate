...
surface_nonterminal_stale() {  # <window> <hash>
  local win=$1 h=$2 key task last surfacedf
  key=$(printf '%s' "$win" | tr ':/.' '___')
  surfacedf="$STATE/.stale-surfaced-$key"
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && ! window_is_busy "$w" "$tail40"; then
        ...
        if [ "$kind" = secondmate ]; then
          ...
          #   - terminal: an authoritative run-step superseded the older pause, so
          #     surface this stale episode once;
            ...
            if [ -e "$pf" ] || status_is_paused_or_captain_held "$(last_status_line "$STATE/$task.status")"; then
              case "$(pause_state_class "$w" "$task")" in
                ...
                working) rm -f "$pf" "$STATE/.paused-resurfaced-$key"
                         ...
                         triage_log "absorbed non-terminal stale (provably working): $w" ;;
                terminal) surface_nonterminal_stale "$w" "$h" ;;
                *)        handle_paused_stale "$w" "$task" "$h" ;;
              esac
            else
