...
spawn_herdr_presentation_order_lock_acquire() {
  local session=${1:-} attempt attempt_limit lock_path wait_seconds
  [ -n "$session" ] || session=$(fm_backend_herdr_session)
  lock_path=$(fm_backend_herdr_presentation_session_lock_path "$session") || return 1
  HERDR_PRESENTATION_ORDER_LOCK="$lock_path"
  wait_seconds=${FM_BACKEND_HERDR_PRESENTATION_LOCK_WAIT_SECONDS:-5}
  case "$wait_seconds" in
    [1-9]|[1-9][0-9]|[1-9][0-9][0-9]) : ;;
    *) wait_seconds=5 ;;
  esac
  attempt_limit=$((wait_seconds * 10))
  attempt=0
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  ...
  for _ in $(seq 1 60); do
    ...
    if [ -n "$p" ]; then
      ...
      # project worktree, so keep polling instead of treating it as a candidate.
      if [ "$p_real" != / ] && [ "$p_real" != "$PROJ_ABS_REAL" ]; then
